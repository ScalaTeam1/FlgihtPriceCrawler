...
object MonadOps {
  ...
  def flatten[X](xyf: Future[Try[X]])(implicit
      executor: ExecutionContext
  ): Future[X] = for (xy <- xyf; x <- asFuture(xy)) yield x
  ...
  def flatten[X](
      xsfs: Seq[Future[Seq[X]]]
  )(implicit ec: ExecutionContext): Future[Seq[X]] = Future.sequence(xsfs) map {
    _ flatten
  }

  def flattenRecover[X](
      esf: Future[Seq[Either[Throwable, Seq[X]]]],
      f: => Throwable => Unit
  )(implicit executor: ExecutionContext): Future[Seq[X]] = {
    def filter(uses: Seq[Either[Throwable, Seq[X]]]): Seq[X] = {
      val uses2 = for {
        use <- uses; if (use match {
  ...
  def flatten[K, V](voKm: Map[K, Option[V]]): Map[K, V] = for (
    (k, vo) <- voKm; v <- vo
  ) yield k -> v
  ...
  def sequence[X](
      xf: Future[X]
  )(implicit executor: ExecutionContext): Future[Either[Throwable, X]] =
    xf transform ({ s => Right(s) }, identity) recoverWith [Either[
      Throwable,
      X
    ]] { case f => Future(Left(f)) }

  // Hint: write as a for-comprehension, using the method sequence (above).
  // 6 points.
  def mapFuture[X](xfs: Seq[Future[X]])(implicit
      executor: ExecutionContext
  ): Seq[Future[Either[Throwable, X]]] = for (xf <- xfs) yield sequence(xf)
  ...
  def sequence[X](xys: Seq[Try[X]]): Try[Seq[X]] =
    xys.foldLeft(Try(Seq[X]()))(combineStrict(_, _)(_ => ()))

  // TODO Fix deprecation
  def sequenceWithLogging[X](
      xys: Seq[Try[X]]
  )(onSubsequentFailure: Throwable => Unit): Try[Seq[X]] =
    xys.foldLeft(Try(Seq[X]()))(combineStrict(_, _)(onSubsequentFailure))

  // TODO Fix deprecation
  def sequence[X](xys: LazyList[Try[X]]): Try[LazyList[X]] =
    xys.foldLeft(Try(LazyList[X]())) { (xsy, xy) =>
      for (xs <- xsy; x <- xy) yield xs :+ x
    }

  // TODO Fix deprecation
  def sequenceForgivingWithLogging[X](xys: Seq[Try[X]])(
      onFailure: Throwable => Unit
  ): Try[Seq[X]] = xys.foldLeft(Try(Seq[X]()))(combineLax(_, _)(onFailure))

  // TODO Fix deprecation
  def sequenceForgiving[X](xys: Seq[Try[X]]): Try[Seq[X]] =
    xys.foldLeft(Try(Seq[X]()))(
      combineLax(_, _)(e =>
        System.err.println(s"ignoring exception ${e.getLocalizedMessage}")
      )
    )

  // TODO Fix deprecation
  def sequence[X](xos: Seq[Option[X]]): Option[Seq[X]] =
    xos.foldLeft(Option(Seq[X]())) { (xso, xo) =>
      for (xs <- xso; x <- xo) yield xs :+ x
    }
  ...
  def zip[A, B](ao: Option[A], bo: Option[B]): Option[(A, B)] = for (
    a <- ao; b <- bo
  ) yield (a, b)

  def zip[A, B](ao: Try[A], bo: Try[B]): Try[(A, B)] = for (a <- ao; b <- bo)
    yield (a, b)
  ...
  def optionToTry[X](xo: Option[X], t: => Throwable): Try[X] =
    Try(xo.get).recoverWith { case _: java.util.NoSuchElementException =>
      Failure[X](t)
    }

  def optionToTry[X](xo: Option[X]): Try[X] = Try(xo.get)

  def map2[T1, T2, U](ty1: Try[T1], ty2: Try[T2])(f: (T1, T2) => U): Try[U] =
    for { t1 <- ty1; t2 <- ty2 } yield f(t1, t2)

  def liftTry[T, U](f: T => U): Try[T] => Try[U] = _ map f

  def liftOption[T, U](f: T => U): Option[T] => Option[U] = _ map f

  def liftFuture[T, U](f: T => U)(implicit
      executor: ExecutionContext
  ): Future[T] => Future[U] = _ map f
  ...
  /** Strict form of combine.
    ...
    * @return a Try of Seq of X.
    */
  private def combineStrict[X](xsy: Try[Seq[X]], xy: Try[X])(
      onFailure: Throwable => Unit
  ): Try[Seq[X]] = xsy match {
    case Success(xs) =>
      xy match {
  ...
  /** Lax form of combine.
    ...
    * @return a Try of Seq of X.
    */
  private def combineLax[X](xsy: Try[Seq[X]], xy: Try[X])(
      onFailure: Throwable => Unit
  ): Try[Seq[X]] = xsy match {
    case Success(xs) =>
      xy match {
    ...
    case Failure(e) =>
      Failure(
        new Exception(
          s"combineLax: logic error: Failure(${e.getLocalizedMessage})"
        )
      )
  }
}
