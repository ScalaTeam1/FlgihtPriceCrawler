...
import edu.neu.coe.csye7200.flightPricePrediction.webCrawler.MonadOps.{
  asFuture,
  flatten,
  liftFuture,
  liftOption,
...
class MonadOpsSpec
  ...
  it should "work" in {
    ...
    val xys: Seq[Try[Int]] =
      Seq(Try(1), Success(2), Failure(WebCrawlerException("dummy")))
    val xsy: Try[Seq[Int]] =
      sequenceForgivingWithLogging(xys)(e => sb.append(e.getLocalizedMessage))
  ...
  it should "work" in {
    val sb = new StringBuilder
    val xys: Seq[Try[Int]] = Seq(
      Try(1),
      Success(2),
      Failure(WebCrawlerException("dummy1")),
      Success(3),
      Failure(WebCrawlerException("dummy2"))
    )
    val xsy: Try[Seq[Int]] =
      sequenceWithLogging(xys)(e => sb.append(e.getLocalizedMessage))
    xsy should matchPattern {
      case Failure(WebCrawlerException("dummy1", null)) =>
    }
  ...
  "zip(Try,Try)" should "succeed" in {
    val (one, two, fail) =
      (Success(1), Success(2), Failure(new NoSuchElementException))
  ...
  it should "work1" in {
    optionToTry(Some(1), WebCrawlerException("junk")) should matchPattern {
      case Success(1) =>
    }
    optionToTry(None, WebCrawlerException("junk")) should matchPattern {
      case Failure(WebCrawlerException("junk", null)) =>
    }
  }
  ...
  it should "work2" in {
    ...
    sequence(Failure(WebCrawlerException("junk"))) should matchPattern {
      case Left(WebCrawlerException("junk", null)) =>
    }
  }
  ...
  it should "work3" in {
    ...
    sequence(
      Seq(Success(1), Failure(WebCrawlerException("junk")))
    ) shouldBe Failure(WebCrawlerException("junk"))
  }
  ...
  it should "work5" in {
    ...
    sequence(Left(WebCrawlerException("junk"))) should matchPattern {
      case None =>
    }
  }
  ...
  it should "work3" in {
    val xf: Future[Seq[Int]] =
      flatten(Seq(Future(Seq(1, 2)), Future(Seq(3, 4))))
